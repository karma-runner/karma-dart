...
var serveStaticFile = function(file, response, process) {
  fs.readFile(file, function(error, data) {
    ...
    // set content type
    var m = mime.lookup(file, 'text/plain');
    if (file.indexOf('.dart', file.length - '.dart'.length) !== -1) {
      m = 'application/dart';
    }
  ...
  webServer.updateFilesPromise = function(promise) {
    ...
    promise.then(function(resolvedFiles) {
      ...
      customFileHandlers.push({
        urlRegex: /\/base\/__adapter_dartacular.dart/,
      ...
      resolvedFiles.included.unshift({
        path: '/base/__adapter_dartacular.dart',
        isUrl: true
      });
